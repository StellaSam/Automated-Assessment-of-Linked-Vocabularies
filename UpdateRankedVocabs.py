...
def write5StarSheet(ws, currentInfo, updateInfo, row):
	...
	ws[getIndex.fInd]=currentInfo.retStarRank
...
def collect4StarInfo(vocab, vFile, vwb, vws):
	...
	class ret4StarInfo:
		def _init_ (self, retVName, retVocab, retToLinksList, retFHasMeta, retStarRank):
			self.retVName=""
			self.retVocab=""
			self.retToLinksList=[]
	...
	if (vocab.find("Base URL not defined") != -1):
		vocab=vocab[22:len(vocab)-1]
	...
	if (vws[getIndex.bInd].value == vocab):
		...
		vStarRank=vws[getIndex.fInd].value
		vStarRank+='*'
		#vws[getIndex.fInd]=vws[getIndex.fInd].value
		vws[getIndex.fInd]=vStarRank
		vwb.save(vFile)
		#vStarRank=vws[getIndex.fInd].value
		# print("VStarRank: " + vStarRank)
		# input()
		...
		ret4StarInfo.retStarRank=vStarRank
	else:
		ret4StarInfo.retVName=None
		ret4StarInfo.retVocab=None
		ret4StarInfo.retToLinksList=[]
		ret4StarInfo.retFHasMeta=None
		ret4StarInfo.retStarRank=None
	return(ret4StarInfo)
	
def getStarInfo(rws, iStarRank, row):
	class retStarInfo:
		def _init_ (self, retVName, retVocab, retToLinksList, retFHasMeta, retStarRank, retRow):
			self.retVName=""
			self.retVocab=""
			self.retToLinksList=[]
			self.retFHasMeta=False
	...
	while ((row <= maxRow) and (rws[getIndex.fInd].value != iStarRank)):
		row += 1
		getIndex=setIndices(row)

	if (rws[getIndex.fInd].value == iStarRank):
		...
		retStarInfo.retStarRank=rws[getIndex.fInd].value+'-'
...
def writeOtherStarsInfo(rFile, rws, rws1, writeRow, iStarRank):
	maxRow=rws.max_row
	currentRow=2
	while (currentRow <= maxRow):
		getUpdateInfo=getStarInfo(rws, iStarRank, currentRow)
